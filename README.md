🤖 GAS ファイル & ワークフロー自動化ツール (GAS File & Workflow Automator)
概要
本プロジェクトは、二つの既存アプリケーション（GAS File Organizer および GAS Workflow Builder）の機能を統合し、よりモダンで拡張性の高い「モジュール駆動型のワークフロー自動化プラットフォーム」として再構築することを目的としています。
...
ベースとなったアプリケーション
本プロジェクトは、以下の二つのアプリケーションを統合したものです。（Libsとして紐づけ済み）
GAS File Organizer: ファイルの絞り込み、変換、アーカイブ機能（主にドライブ操作モジュールに統合）
GAS Workflow Builder: ワークフロー構造、実行制御、トリガー、予約実行機能
